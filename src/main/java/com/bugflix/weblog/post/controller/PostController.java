...
public class PostController {
    ...
    public ResponseEntity<PostPreviewProfileResponse> getPostPreviewWithProfile(@PathVariable Long postId,
                                                              @AuthenticationPrincipal UserDetails userDetails) {
        return ResponseEntity.ok(postServiceImpl.getPostPreviewWithProfile(postId, userDetails));
    }
    ...
    /**
     * (프로필) 특정 Page 내의 Post 미리보기 전체를 조회합니다.
     *
     * @param url Post 미리보기를 조회할 Page의 Url
     * @param userDetails 조회를 요청한 사용자의 정보;
     *                    사용자에게 특정 Page 내의 Post 미리보기 조회 권한이 있는지 검사하기 위해 필요합니다.
    ...
    public ResponseEntity<List<PostPreviewProfileResponse>> getPostPreviewWithProfile(@RequestParam String url,
                                                                    @AuthenticationPrincipal UserDetails userDetails) {
        if (userDetails == null) return ResponseEntity.ok(postServiceImpl.getPostPreviewsWithProfile(url));
        return ResponseEntity.ok(postServiceImpl.getPostPreviewsWithProfile(url, userDetails));
    }
    ...
    /**
     * (페이징, 프로필) 특정 Page에서 현재 사용자의 Post 미리보기를 조회합니다.
     *
     * @param url 특정 Page의 url
     * @param offset 조회 시작 인덱스
     * @param limit 한 번에 조회할 갯수
     ...
     * @return 특정 Page에서 현재 사용자의 Post 미리보기 전체 List
     */
    @GetMapping("/v3/posts/mine")
    @Operation(summary = "내 Post 미리보기 목록 조회 Ver.3", description = "url을 포함하는 경우 해당 주소에 작성된 포스트 내용을, " +
                                                                  "포함되어 있지 않은 경우에는 내가 작성한 포스트 목록을 반환합니다." +
                                                                  "작성자 프로필을 포함합니다.")
    public ResponseEntity<List<PostPreviewProfileResponse>> getMyPostPreviewWithProfile(@RequestParam(name = "url", required = false) String url,
                                                                                @RequestParam Integer offset,
                                                                                @RequestParam Integer limit,
                                                                                @AuthenticationPrincipal UserDetails userDetails) {
        if (url == null) return ResponseEntity.ok(postServiceImpl.getMyPostPreviewWithProfile(userDetails, offset, limit));
        return ResponseEntity.ok(postServiceImpl.getMyPostPreviewWithProfile(url, userDetails, offset, limit));
    }
    ...
    /**
     * (페이징, 프로필) 다른 사용자의 Post 미리보기를 조회합니다.
     *
     * @param offset 조회 시작 인덱스
     * @param limit 한 번에 조회할 갯수
     * @param nickname 검색할 사용자 이름
    ...
    public ResponseEntity<List<PostPreviewProfileResponse>> getOthersPostPreviewWithProfile(@RequestParam Integer offset,
                                                                                    @RequestParam Integer limit,
                                                                                    @PathVariable String nickname) {
        return ResponseEntity.ok(postServiceImpl.getOthersPostPreviewWithProfile(nickname, offset, limit));
    }
    ...
    public ResponseEntity<List<PostPreviewProfileResponse>> getPopularPostsWithProfile(@Valid @ModelAttribute PostPopularRequest postPopularRequest,
        ...
        return ResponseEntity.ok(postServiceImpl.getPopularPostsWithProfile(postPopularRequest, userDetails));
    }
